...
static boolean_t associate_node_to_network(mlmeMessage_t *pMsgIn);
static void node_init();
static uint8_t node_search_free_index(void);
...
static node_t node[QNodesAllowed];

static uint8_t g_current_index;
...
{ 
    ...
    while(1)
    {
        ...
        case stateInit:
			...
			Serial_Print(interfaceId, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\r", gAllowToBlock_d);
			Serial_Print(interfaceId, "~~                                        ~~\n\r", gAllowToBlock_d);
			Serial_Print(interfaceId, "~~           Beacon  (Coordinator)        ~~\n\r", gAllowToBlock_d);
      ...
      case stateScanEdWaitConfirm:
          ...
          if (ev & gAppEvtMessageFromMLME_c)
          {
              if (pMsgIn)
              {
            	  /* verify if node is already associated to the network */
            	  /*		TODO:		*/
      ...
      if (ev & gAppEvCounter1_c)
      {
    	  TurnOffLeds();
    	  Led1On();
      }

      if (ev & gAppEvCounter2_c)
      {
    	  TurnOffLeds();
    	  Led2On();
      }

      if (ev & gAppEvCounter3_c)
      {
    	  TurnOffLeds();
    	  Led3On();
      }

      if (ev & gAppEvCounter4_c)
      {
    	  TurnOffLeds();
    	  Led4On();
      }
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  if(pMsg != NULL)
  {
	  NodeIsAssociated_b = associate_node_to_network(&pMsgIn);

	  if(TRUE == NodeIsAssociated_b)
	  {
		...
		if(pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoAllocAddr_c)
		{
		  ...
		  pAssocRes->assocShortAddress = node[g_current_index].short_address;
		}
		else
		{
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
    ...
    FLib_MemCpy((uint8_t*) g_buffer, pMsgIn->msgData.dataInd.pMsdu, mDefaultBufferSize);
    ...
    if(g_buffer[0] == 'c'  &&
	   ...
	   g_buffer[4] == 't')
    {
        g_counter = g_buffer[10];

    	switch(g_counter)
    	{
			case (counter_state_1):
				OSA_EventSet(mAppEvent, gAppEvCounter1_c);
			break;
			case (counter_state_2):
				OSA_EventSet(mAppEvent, gAppEvCounter2_c);
			break;
			case (counter_state_3):
				OSA_EventSet(mAppEvent, gAppEvCounter3_c);
			break;
			case (counter_state_4):
				OSA_EventSet(mAppEvent, gAppEvCounter4_c);
			break;
    	}
    	...
    	Serial_PrintHex(interfaceId,(uint8_t*)&pMsgIn->msgData.dataInd.mpduLinkQuality,1 ,0x00);
    	Serial_Print(interfaceId, "\n\rPayload Size.....0x", gAllowToBlock_d);
    	Serial_PrintHex(interfaceId,(uint8_t*)&pMsgIn->msgData.dataInd.msduLength, 1 ,0x00);
		Serial_Print(interfaceId, "\n\r~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\r", gAllowToBlock_d);
...
static void node_init()
{
	...
	for(i=0; i < QNodesAllowed; i++)
	{
		node[i].IsNodeSet = FALSE;

		switch(i)
		{
		case 0:
			node[i].short_address = 0x0005;
		break;
		case 1:
			node[i].short_address = 0x0002;
		break;
		case 2:
			node[i].short_address = 0x0003;
		break;
		case 3:
			node[i].short_address = 0x0004;
		break;
		case 4:
			node[i].short_address = 0x0005;
		break;
...
{
	uint8_t index;
	boolean_t nodeIsAssociated_b = FALSE;
	boolean_t* pointer_to_attribute;
	index = node_search_free_index();
	g_current_index = index;

	if(QNodesAllowed > index)
	{
		node[index].IsNodeSet = TRUE;
		//node[index].DeviceType = (((nwkMessage_t *)pMsgIn)->msgData.beaconNotifyInd.);
		nodeIsAssociated_b = TRUE;

		((mlmeMessage_t *)pMsgIn)->msgType = gMlmeGetReq_c;
		((mlmeMessage_t *)pMsgIn)->msgData.getReq.pibAttribute = gMPibRxOnWhenIdle_c;
		((mlmeMessage_t *)pMsgIn)->msgData.getReq.pibAttributeValue = &pointer_to_attribute;
	    (void)NWK_MLME_SapHandler(((mlmeMessage_t *)pMsgIn), macInstance);

	    if(*pointer_to_attribute > 1)
	    {
	    	node[index].RxOnIdle = TRUE;
	    	node[index].DeviceType = FFD;
	    }
	    else
	    {
	    	node[index].RxOnIdle = TRUE;
	    	node[index].DeviceType = RFD;
	    }
...
//		(void)NWK_MLME_SapHandler( pMsg, macInstance );
	...
	return nodeIsAssociated_b;
}

static uint8_t node_search_free_index(void)
{
	uint8_t i;
	uint8_t node_free_index;
	boolean_t node_is_free_b;

	node_is_free_b = FALSE;

	for(i = 0; i < QNodesAllowed; i++)
	{
		if(FALSE == node[i].IsNodeSet)
		{
			node_free_index = i;
			node_is_free_b = TRUE;
			break;
	...
	if(TRUE == node_is_free_b)
	{
		return node_free_index;
	}
	else
	{
		return NETWORK_IS_NODE_FULL;
	}
}
